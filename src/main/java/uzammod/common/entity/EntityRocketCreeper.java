...
import uzammod.common.entity.ai.EntityAIRocketCreeperSwell;

public class EntityRocketCreeper extends EntityModMobBase
{
	/**
	...
	public EntityRocketCreeper(World p_i1733_1_)
	{
		super(p_i1733_1_);
	...
	public boolean attackEntityFrom(DamageSource ds, float damage)
	{
		...
		int r = 2 * this.worldObj.difficultySetting.ordinal();
		if( ret && ds != ds.fall && target instanceof EntityLivingBase )
		{
			if( target.ridingEntity == null &&
				this.worldObj.difficultySetting.ordinal() >= EnumDifficulty.NORMAL.ordinal() &&
				this.getDistanceSqToEntity(target) < r * r )
			{
				firework(target);
			}
	...
	public void firework(Entity target)
	{
		...
		{
			nbtFireworks.setByte("Flight", (byte) (1));
		}
		else
		{
			nbtFireworks.setByte("Flight", (byte) (1 + rand.nextInt(2)));
		}

		NBTTagList nbttaglist = new NBTTagList();
		NBTTagCompound nbt = new NBTTagCompound();
		nbt.setBoolean("Flicker", rand.nextBoolean());
		nbt.setBoolean("Trail", rand.nextBoolean());
		nbt.setByte("Type", (byte) rand.nextInt(5));
		nbt.setIntArray("Colors", new int[] { 0xFF0000, 0x00FF00, 0x0000FF });
		nbt.setIntArray("FadeColors", new int[] { 0xFF00FF, 0xFFFF00, 0xFF00FF });
		nbttaglist.appendTag(nbt);
		nbtFireworks.setTag("Explosions", nbttaglist);

		NBTTagCompound nbtItem = new NBTTagCompound();
		nbtItem.setTag("Fireworks", nbtFireworks);
		item.setTagCompound(nbtItem);
		...
		entityfireworkrocket.motionX *= 10;
		entityfireworkrocket.motionZ *= 10;

		this.worldObj.spawnEntityInWorld(entityfireworkrocket);

		target.mountEntity(entityfireworkrocket);
	}
	...
	{
		return rand.nextInt(3)==0? Items.firework_charge: Items.fireworks;
	}
